...
export default function IncomeCard() {
  ...
  const CurvedChart = ({ data, color = "currentColor" }) => {
    ...
    const values = data.map(d => d.earning);
  ...
  return (
    <Card width="full" padding="none" title="Income" className="h-full relative overflow-hidden group rounded-3xl">
      ...
      <div className="absolute inset-x-0 bottom-0 h-32 z-0 text-emerald-500 opacity-20 pointer-events-none translate-y-4">
        <CurvedChart data={incomeData?.history || []} color="currentColor" />
      </div>
    </Card>
  );
}

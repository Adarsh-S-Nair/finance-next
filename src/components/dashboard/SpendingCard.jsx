...
export default function SpendingCard() {
    ...
    const fetchSpendingData = async () => {
      ...
      try {
        ...
        if (data.data) {
          ...
          const spendingTrend = last6Months.map(m => m.spending);
          setTrendData(spendingTrend);

          const currentMonth = data.data[data.data.length - 1];
          setSpending(currentMonth?.spending || 0);

          if (data.momComparison) {
            setMomChange(data.momComparison.spendingChange);
          }
        }
      } catch (error) {
  ...
  const { main, cents } = formatCurrencyParts(spending);
    ...
    >
      <div className="p-6 relative z-10">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-sm font-medium text-[var(--color-muted)]">Spending</h3>

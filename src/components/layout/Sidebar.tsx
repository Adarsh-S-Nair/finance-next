...
export default function Sidebar({ isCollapsed, toggle, showToggle }: { isCollapsed?: boolean; toggle?: () => void; showToggle?: boolean }) {
  return (
    <aside
      className={`hidden md:flex flex-col fixed top-0 left-0 h-screen z-50 transition-all duration-300 ease-in-out border-r border-[var(--color-fg)]/[0.06] ${isCollapsed ? 'w-20' : 'w-72'
        }`}
    >

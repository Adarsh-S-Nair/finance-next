...
/**
 * Fixed bottom bar that spans both the household rail and the main sidebar.
 * Clicking the profile row animates an options panel upward that also covers
 * the combined width — so Settings / Upgrade / Log out visually overlay the
 * rail + sidebar, not just the sidebar.
 */
export default function ProfileBar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <>
      <div
        ref={containerRef}
        className="hidden md:flex flex-col fixed bottom-0 left-0 md:w-40 xl:w-80 z-[60]"
      >
        <AnimatePresence initial={false}>
          {expanded && (
            <motion.div
              ...
              className="overflow-hidden border-r border-t border-[var(--color-fg)]/[0.06] bg-[var(--color-sidebar-bg)]"
            >
              <div className="py-1.5">
                {tier === "free" && (
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-4 py-2 text-[13px] text-[var(--color-accent)] hover:bg-[var(--color-accent)]/10"
                  >
                ...
                <div className="flex items-center gap-2.5 px-4 py-2 text-[13px] text-[var(--color-muted)] opacity-40 cursor-not-allowed">
                ...
                <button
                  onClick={onLogout}
                  className="w-full flex items-center gap-2.5 px-4 py-2 text-[13px] text-[var(--color-muted)] hover:text-[var(--color-fg)] hover:bg-[var(--color-fg)]/[0.05]"
                >
        ...
        <button
          ...
          className={clsx(
            "h-16 w-full flex items-center gap-3 px-5 text-left cursor-pointer group border-t border-r border-[var(--color-fg)]/[0.06] bg-[var(--color-sidebar-bg)]",
            expanded
        ...
        >
          ...
          <div className="flex-1 min-w-0 hidden xl:block">
            ...
            {tier === "pro" && (
              <p className="text-[10px] font-semibold tracking-wide uppercase text-[var(--color-accent)] leading-tight mt-0.5">
                Pro
              </p>
            )}
          </div>

          <LuChevronsUpDown className="h-3.5 w-3.5 text-[var(--color-muted)]/50 flex-shrink-0 hidden xl:block group-hover:text-[var(--color-muted)]" />
        </button>
      </div>

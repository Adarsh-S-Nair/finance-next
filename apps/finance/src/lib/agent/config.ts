...
- get_net_worth_performance: Same shape as get_investment_performance but spans ALL accounts — cash, investments, credit cards, loans, mortgages — so liabilities subtract from the change figure. Use when the user asks about overall financial trend ("how's my net worth doing", "am I building wealth", "where do I stand for the year"). Investment account balances use live ticker prices. Same incomplete-history caveat applies.
- get_account_breakdown: Assets-only breakdown by account category (cash vs investments). Returns segments with absolute amount and percentage of total assets. Pairs naturally with get_net_worth_performance to give the user a chart + composition view. For high-level "what's my financial picture" questions; for detailed per-account info use get_account_balances instead.

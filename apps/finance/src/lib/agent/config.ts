...
3. **Read the response, in this order:**
   ...
   - **For each stream, check has_outlier**. If true, the bucket contains a deposit much larger than the others (a bonus or RSU vest). The tool already excluded it from streams_only — your job is to MENTION it: "I see one $5,193 deposit on Apr 13 that's bigger than your regular ~$3,300 paychecks; looks like a bonus or RSU vest. I excluded it from the recurring number. If it actually repeats, your monthly is closer to streams_only.monthly_average_with_outliers." Use typical_amount as the "your regular paycheck is $X" reference.

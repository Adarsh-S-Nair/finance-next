...
export const POST = withAuth('detect-similar', async (request, userId) => {
  const { transactionId, categoryId } = (await request.json()) as RequestBody;
  ...
  if (fetchError || !transaction) {
    logger.error('Transaction not found', fetchError as unknown as Error, { transactionId });
  ...
  if (accountError || !account || account.user_id !== userId) {
    logger.error('Unauthorized access to transaction', null, {
      transactionId,
      userId,
  ...
  if (searchError) {
    logger.error('Error searching similar transactions (exact)', searchError as unknown as Error);
    throw searchError;
  }

  let matchType: 'exact' | 'name' = 'exact';
  ...
  if (!similarTransactions || similarTransactions.length === 0) {
    ...
    if (nameOnlyError) {
      logger.error(
        'Error searching similar transactions (name only)',
        nameOnlyError as unknown as Error
      );
      throw nameOnlyError;
    }
  ...
  type SimilarTx = (NonNullable<typeof similarTransactions>)[number];

  // 6. Transform data to match frontend expectations (flatten category info)
  const transformedTransactions = (similarTransactions ?? []).map((tx: SimilarTx) => ({
    ...tx,
  ...
  const criteria = {
    ...
    operator: transaction.merchant_name ? 'is' : 'contains',
    matchType,

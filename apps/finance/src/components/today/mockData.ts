...
/** Severity of a decision — drives the color/icon/label on the cards. */
export type FeedSeverity = "action" | "review" | "info";
...
export interface FeedItem {
  id: string;
  tone: FeedTone;
  /** Decision severity — controls the status icon, color, and label. */
  ...
  body: string;
  /** Short supporting line shown on the dashboard card (savings, totals…). */
  meta?: string;
  evidence?: EvidenceRow[];
  /** Primary call to action — only on decision items that need one. */
  action?: string;
  /** Shown after the user approves the action. */
  confirmation?: string;
  /** Rough annual dollar value of acting on this. */
  stakes?: number;
}
...
export const FEED_ITEMS: FeedItem[] = [
  {
    ...
    when: "Today",
    headline: "You're paying for Creative Cloud on two cards",
    body:
      "A second $52.99 Adobe subscription started in March, right when you switched plans — the old one never stopped billing. That's $636/yr if it keeps running.",
    meta: "Potential savings: $636/year",
    evidence: [
  ...
  },
  {
    ...
    category: "Savings",
    when: "Today",
    headline: "Your emergency fund now covers 6.3 months of expenses",
    body:
      "Two months ago it was 4.1. At your current spending you're comfortably past the 6-month mark advisors usually suggest — nothing to do here.",
    meta: "Great job staying prepared",
  },
];
...
export const HANDLED_ITEMS: FeedItem[] = [
  {
    id: "categorized",
    tone: "handled",
    ...
    body:
      "Three were ambiguous — I filed the Costco run under Groceries based on your history, not Shopping.",
  },
  {
    id: "refund-matched",
    tone: "handled",
    ...
    body: "Matched it to the standing desk mat you returned on May 30.",
  },
];

// Combined list kept for the /today feed, which renders decisions and
// handled work together.
export const ALL_FEED_ITEMS: FeedItem[] = [...FEED_ITEMS, ...HANDLED_ITEMS];

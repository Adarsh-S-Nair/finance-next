-- Add hex_color column to system_categories so each category has its own
-- unique muted color instead of inheriting from the parent group.

ALTER TABLE public.system_categories
  ADD COLUMN IF NOT EXISTS hex_color VARCHAR(7);

ALTER TABLE public.system_categories
  ADD CONSTRAINT system_categories_hex_color_format
  CHECK (hex_color IS NULL OR hex_color ~ '^#[0-9A-Fa-f]{6}$');

-- Populate every system_category with a unique muted color.
DO $$
DECLARE
  palette TEXT[] := ARRAY[
...
ALTER TABLE public.system_categories
  ALTER COLUMN hex_color SET DEFAULT '#71717a';

-- Also update category_groups to use muted colors
